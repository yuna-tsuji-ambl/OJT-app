...
> **最終更新**: 2026-07-06  
> **ステータス**: ドラフト（v0.3 — Firestore 永続化確定・実装中）
...
| テスト仕様書 | 機能ごとの正（TDD ワークフロー） |
| 本書の位置づけ | 実装の全体像・API・画面・データモデルの参照用。仕様変更時はテスト仕様書と本書をセットで更新する |
...
| 区分 | 内容 |
|------|------|
| 対象ユーザー | 新卒（trainee）、OJT トレーナー（trainer） |
| 対象環境 | ローカル開発、Docker、Google Cloud Run（想定） |
| 永続化 | **Firestore**（`DB_PROVIDER=firestore`）。テスト・未設定時はインメモリ |
| 認証 | **モック**（ロール選択式ログイン、HTTP ヘッダーによる擬似認可） |
| 外部連携 | Google Sheets API は **未実装**（`SheetRepository` インターフェースのみ） |

### 2.3 追加予定機能（v0.2 設計済み・未実装）

| 優先 | 機能 | 概要 | 詳細 |
|------|------|------|------|
| P0 | 永続化 | インメモリ → DB 移行 | §9.4 |
| P1 | 課題管理（トレーナー入力） | スプシ代替。トレーナー CRUD → 新卒閲覧・申請 | §7.4 |
| P1 | 日次・週次報告書 | 振り返りテンプレートの作成・提出・閲覧 | §7.5 |
| P2 | 目標・ガント管理 | 期間付きタスク登録とガントチャート表示 | §7.6 |
| P2 | 学び共有 | 日次の学習メモ + リンクのタイムライン | §7.7 |
...
| フェーズ | 内容 | 成果物 |
|----------|------|--------|
| **Phase 0** | DB 選定確定、Firestore Emulator 基盤 | `firebase.json`, `createPersistence` |
| **Phase 1** | 既存 3 機能の DB 移行 | condition, status, chat, quest の Firestore repo |
| **Phase 2** | 課題管理（§7.4） | Assignment CRUD + 既存申請フロー接続 |
| **Phase 3** | 報告書（§7.5） | 日次・週次 API + 画面 |
| **Phase 4** | 学び共有（§7.7） | タイムライン API + 画面 |
| **Phase 5** | ガント（§7.6） | Goal CRUD + Gantt UI |
| **Phase 6** | BigQuery エクスポート（任意） | 週次バッチ、分析ダッシュボード |
